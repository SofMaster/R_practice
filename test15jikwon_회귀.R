# 원격 DB(MariaDB)와 연동하여 자료를 읽은 후 jkiwon 테이블로 근무년수에 따른 연봉 예측 모델 작성
...
cor(jik_data$jik_workyear, jik_data$jik_pay)   # 상관계수 0.9196725
# 인과관계 ? 

model <- lm(formula = jik_pay ~ jik_workyear, data = jik_data)
summary(model)    # p-value : 6.943e-13 < 0.05 유의한 모델. R-squared: 0.8458(독립변수가 종속변수를 85%정도 설명하고 있다.)
