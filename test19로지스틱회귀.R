...
# 분류결과는 이항분포를 따른다. 출력범위가 0 ~ 1 사이가 됨.
# 출력을 위해 로지스틱함수(시그모이드 함수)를 사용한다.
# 예 : 생존 / 사망, 고객 부도 여부, 최종학력 등 범주형으로 결과를 분류하고자 할때 사용


# 시험성적, 내신성적, 순위에 따른 대학원 합격 여부 데이터로 분류 모델 작성
...
# 분류 모델 작성
# train / test 분리 : 모델과의 과적합(overfitting) 방지가 목적
set.seed(1)
...
model <-lm(formula = admit ~ ., data = train)
summary(model)
predict(model)

# 로지스틱 회귀모델은 일반화된 선형회귀 모델
# model <- lm(formula = admit ~ ., data = train)  # 선형회귀 모델
lgmodel <- glm(formula = admit ~ ., data = train, family = binomial(link = 'logit'))  # glm : general linear model(일반화된 선형회귀 모델)    family = binomial(link = 'logit')
lgmodel
summary(lgmodel)

# 예측
pred <- predict(lgmodel, newdata = test, type = "response")  # type = "response" : 0 ~ 1 사이로 값이 나옴
...
t
(77 + 11) / nrow(test)  #0.73333  73.3%의 분류 정확도를 갖는다.
(t[1,1] + t[2, 2]) / nrow(test)  # 행과 열을 이용하여 정확도 계산
sum(diag(t)) / nrow(test)  # 주대각으로 계산
