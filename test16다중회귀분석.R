# p -value : 모델의 성능을 나타내는 지수
# Adjusted R-squared : 독립변수가 많을 때 독립변수가 종속변수의 경향(pattern)을 얼마나 설명할  수 있느냐를 나타낸다.
# cor() : 상관 관계의 정도를 나타낸다. 
...
pre_states <- edit(part_states)  # 편집기 열기
pre_states
...
# 1) 잔차의 정규성 검정
shapiro.test(residuals(mfit))   # p-value = 0.6672 > 0.05 정규성 만족 (shapirotest에서는 0.05보다 커야 정규성만족)

# Shapiro-Wilk normality test
# 
# data:  residuals(mfit)
# W = 0.98264, p-value = 0.6672  > 0.05 정규성 만족
...
# 3) 선형성 검정
boxTidwell(Murder ~ Population + Illiteracy , data = states)   # boxTidwell은 독립변수를 최대 2개까지만 입력 가능하다.

#            MLE of lambda Score Statistic (z) Pr(>|z|)        # MLE : 우도
...
# 4) 등분산성 검정
ncvTest(mfit)   # p = 0.18632 > 0.05 이므로 등분산성 만족
...
# 예측 (독립, 설명)변수 선택 : 분석가의 입장에서 가장 고민해야할 문제 중 하나
...
min_model <- lm(Murder ~ 1, data = states)   # 변수없이 추가할 때 1
fwd_model <- step(full_model, direction = "forward",
